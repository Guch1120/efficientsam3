#!/usr/bin/env python3
"""ROS1 wrapper node for EfficientSAM3 image segmentation.
...
Default behavior uses a center positive point prompt for fast interactive-free demos.
"""
...
class EfficientSam3Ros1Node:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.sub = rospy.Subscriber(args.input_topic, RosImage, self._on_image, queue_size=1)
        self.pub = rospy.Publisher(args.output_topic, RosImage, queue_size=1)

        rospy.loginfo("EfficientSAM3 ROS1 node initialized.")

    def _on_image(self, msg: RosImage) -> None:
        try:
            bgr = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
        except CvBridgeError as err:
            rospy.logerr(f"cv_bridge conversion failed: {err}")
            return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="EfficientSAM3 ROS1 wrapper node")
...
def main() -> None:
    rospy.init_node("efficientsam3_ros1")
    args = parse_args()
    _ = EfficientSam3Ros1Node(args)
    rospy.spin()
